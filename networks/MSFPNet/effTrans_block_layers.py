...
class MlpChannel(nn.Module):
    def __init__(self, hidden_size, mlp_dim):
        super().__init__()
        self.fc1 = nn.Conv3d(hidden_size, 2*mlp_dim, 1)
        self.act = nn.GELU()
        self.fc2 = nn.Conv3d(2*mlp_dim, mlp_dim, 1)
...
class effTrans_layers(nn.Module):
    ...
    def __init__(self, embed_dim, width_multiplier=2.0, dextra_depth=4, dextra_proj=2, heads=8, sr_ratio=2):
        ...
        self.mlp=MlpChannel(self.embed_dim,self.embed_dim)
        ...
        self.norm = get_norm_layer(name='ln', out_features=self.embed_dim)
    def __repr__(self):
